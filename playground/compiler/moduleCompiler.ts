...
function processFile(file: File, seen = new Set<File>()) {
    ...
    function defineImport(node: Node, source: string) {
        ...
        if (!(filename in store.files)) throw new Error(`File "${filename}" does not exist.`);
    ...
    s.prepend(`const ${moduleKey} = __modules__[${JSON.stringify(file.filename)}] = { [Symbol.toStringTag]: "Module" }\n\n`);
    ...
    for (const node of ast) {
        ...
        // default export
        if (node.type === 'ExportDefaultDeclaration') s.overwrite(node.start!, node.start! + 14, `${moduleKey}.default =`);
    ...
    for (const node of ast) {
        ...
        walkIdentifiers(node, (id, parent, parentStack) => {
            ...
            if (isStaticProperty(parent) && parent.shorthand) {
                ...
                if (!(parent as any).inPattern || isInDestructureAssignment(parent, parentStack)) s.appendLeft(id.end!, `: ${binding}`);
    ...
    if (importedFiles.size) {
        for (const imported of importedFiles) processed.push(...processFile(store.files[imported], seen));
    }
...
const isStaticProperty = (node: Node): node is ObjectProperty => node.type === 'ObjectProperty' && !node.computed;

function extractNames(param: Node): string[] {
    return extractIdentifiers(param).map(id => id.name);
}

function extractIdentifiers(param: Node, nodes: Identifier[] = []): Identifier[] {
    switch (param.type) {
        case 'Identifier':
            nodes.push(param);
            break;
