...
export async function setVersion(version: string) {
    ...
    const [compiler] = await Promise.all([import(/* @vite-ignore */ compilerUrl), import(/* @vite-ignore */ runtimeUrl)]);
...
export async function compileFile({ filename, code, compiled }: File) {
    ...
    if (descriptor.template && !descriptor.scriptSetup) {
        const clientTemplateResult = doCompileTemplate(descriptor, id, bindings, false, isTS);
        if (!clientTemplateResult) return;

        clientCode += clientTemplateResult;

        const ssrTemplateResult = doCompileTemplate(descriptor, id, bindings, true, isTS);
...
async function doCompileScript(descriptor: SFCDescriptor, id: string, ssr: boolean, isTS: boolean): Promise<[string, BindingMetadata | undefined] | undefined> {
...
function doCompileTemplate(descriptor: SFCDescriptor, id: string, bindingMetadata: BindingMetadata | undefined, ssr: boolean, isTS: boolean) {
